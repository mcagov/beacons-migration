    ...
    while True:
        ...
        for pk_beacon_owner_id, fk_beacon_id, owner_name, company_name, care_of, address_1, address_2, address_3, address_4, country, post_code, phone_1, phone_2, mobile_1, mobile_2, fax, email, is_main, create_user_id, create_dt, update_user_id, update_dt, versioning in rows:
           ...
           if valid_country == 'UNITED KINGDOM' or valid_country == None:
               ...
               post_code = _extractByRegex(postcode_regex, post_code, uk_postcodes)
               country = _extractByRegex(postcode_regex, country, uk_postcodes)
               address_4 = _extractByRegex(postcode_regex, address_4, uk_postcodes)
               address_3 = _extractByRegex(postcode_regex, address_3, uk_postcodes)
               address_2 = _extractByRegex(postcode_regex, address_2, uk_postcodes)
               address_1 = _extractByRegex(postcode_regex, address_1, uk_postcodes)

           # Try to find all phone numbers - regex is not perfect but good enough for this.
           phone_regex = '[+]*[(]{0,1}[0-9]{1,4}[)]{0,1}[-\s\./0-9]*'
           valid_phone_numbers = []
           phone_1 = _extractByRegex(phone_regex, phone_1, valid_phone_numbers)
           phone_2 = _extractByRegex(phone_regex, phone_2, valid_phone_numbers)
           mobile_1 = _extractByRegex(phone_regex, mobile_1, valid_phone_numbers)
           mobile_2 = _extractByRegex(phone_regex, mobile_2, valid_phone_numbers)
           ...
           insert_cursor.execute(insert_sql, [pk_beacon_owner_id, fk_beacon_id, owner_name, company_name, care_of, address_1, address_2, address_3, address_4, country, post_code, phone_1, phone_2, mobile_1, mobile_2, fax, email, is_main, create_user_id, create_dt, update_user_id, update_dt, versioning])
           insert_cursor.close()
